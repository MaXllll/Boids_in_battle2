...
public class SC_boids_2D : MonoBehaviour {
	...
	void Start()
	{
		...
		{
			GameObject GO_tmp = Instantiate(_Prefab_boid, new Vector3(Random.value * 100 - 50, 1f, Random.value * 100 - 50), Quaternion.Euler(new Vector3(0f, Random.value * 360, 0f))) as GameObject;
	...
	private void CaclulateBoidsTargets(object o_thread)
	{
		...
		for (int i = i_start_boid; i < i_end_boid; ++i)
		{
			...
			V3_target = ProjectVectorOnPlane(Vector3.up, V3_target);
